...
public class SpiderManTutorialGestureDetector : MonoBehaviour
{
    ...
    void Update()
    {
        if (hand == null || skeleton == null || webShooterTutorial == null)
            return;
        ...
        // RECARGA: puño cerrado
        if (fistActive && !fistGestureWasActive && cooldownPassed && reloadWindowPassed)
        {
            if (debugLogs)
                Debug.Log($"{name}: RECARGA detectada en tutorial");

            webShooterTutorial.ActivateReloadFromGesture();
            lastGestureTime = Time.time;
        }

        // DISPARO: gesto Spider-Man
        if (spiderActive && !spiderGestureWasActive && cooldownPassed)
        {
            if (debugLogs)
                Debug.Log($"{name}: DISPARO detectado en tutorial");

            webShooterTutorial.ActivateFromGesture();
            lastGestureTime = Time.time;
